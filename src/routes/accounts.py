...
async def register_user(
    ...
    email_manager: EmailSenderInterface = Depends(get_accounts_email_notificator),
    ...
    try:
        ...
        activation_link = f"{FRONTEND_URL}/register?token={activation_token.token}&email={activation_token.user.email}"

        async def send_email_task(email: str, link: str):
            await email_manager.send_activation_email(
                email=email, activation_link=link
            )
        background_tasks.add_task(send_email_task, new_user.email, activation_link)
...
async def activate_account(
    ...
    login_link = f"{FRONTEND_URL}/activate?token={token_record.token}&email={token_record.user.email}"

    async def send_email(email: str, link: str):
        await email_manager.send_activation_complete_email(
            email=email, login_link=link)
    background_tasks.add_task(send_email, user.email, login_link)
...
async def request_password_reset_token(
    ...
    reset_link = f"{FRONTEND_URL}/password-reset/request/?token={reset_token.token}&email={reset_token.user.email}"

    async def send_email(email: str, link: str):
        await email_manager.send_password_reset_email(email=email, reset_link=link)
    background_tasks.add_task(send_email, user_email, reset_link)
